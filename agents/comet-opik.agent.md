...
1. **Server launch** – Copilot roda `npx -y opik-mcp`; mantenha Node.js ≥ 20.11.
2. **Load credentials**
...
| `COPILOT_MCP_OPIK_API_BASE_URL` | optional | Default `https://www.comet.com/opik/api`; use `http://localhost:5173/api` para OSS |
| `COPILOT_MCP_OPIK_SELF_HOSTED` | optional | `"true"` quando apontar para OSS Opik |
| `COPILOT_MCP_OPIK_TOOLSETS` | optional | Lista com virgulas, ex.: `integration,prompts,projects,traces,metrics` |
| `COPILOT_MCP_OPIK_DEBUG` | optional | `"true"` escreve `/tmp/opik-mcp.log` |
...
## Instrumentation & Prompt Governance
